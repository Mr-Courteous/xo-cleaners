...
export function renderPickupReceiptHtml(ticket: Ticket) {
  ...
  // Balance should ideally be 0 at pickup, but we calculate it dynamically
  const balance = finalTotal - paid;

  // Calculate total pieces
  // Note: Using the logic from your existing template. 
  // If 'pieces' property is missing on items, this might result in 0. 
  // You might want to change (item.pieces || 0) to (item.pieces || 1) if pieces isn't strictly tracked.
  const totalPieces = items.reduce((sum, item) => sum + (item.quantity * (item.pieces || 0)), 0);

  // --- NEW: Capture Current Date for Pickup Time ---
  ...
  return `
    <div style="width:55mm;margin:0 auto;font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif;color:#111;padding:8px;">
      ...
      <div style="margin-top:10px;text-align:center;">
        <div style="display:inline-block;background:#000;color:#fff;padding:6px 12px;border-radius:4px;font-weight:800;">PICKED UP</div>
      </div>

      <div style="margin-top:8px;text-align:center;font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif;font-size:10pt;font-weight: 600;">
        Picked Up On: ${pickedUpDate}
      </div>
      
      <div style="margin-top:8px;text-align:center;font-size:9pt;">
