...
export default function DashboardAnalytics() {
        ...
        const fetchData = async () => {
            ...
            try {
                const [dashboardRes, chartsRes] = await Promise.all([
                    axios.get(`${baseURL}/api/organizations/analytics/dashboard?start_date=${dateRange.start}&end_date=${dateRange.end}`, getAuthHeaders()),
                    axios.get(`${baseURL}/api/organizations/analytics/charts?start_date=${dateRange.start}&end_date=${dateRange.end}`, getAuthHeaders())
                ]);
    ...
    const filteredData = useMemo(() => {
        ...
        // If backend didn't return a payments ledger table, synthesize from tickets' paid_amount
        let finalLedger = ledger;
        if ((!finalLedger || finalLedger.length === 0) && data.tickets && data.tickets.length > 0) {
            finalLedger = data.tickets
                .filter(t => t.paid_amount && t.paid_amount !== 0)
                .map(t => ({
                    id: `synth_${t.id}`,
                    date: t.transfer_timestamp || t.created_at,
                    reference: `Ticket #${t.ticket_number}`,
                    customer_name: ((data.customers.find(c => c.id === t.customer_id)?.first_name || 'Walk-in') + ' ' + (data.customers.find(c => c.id === t.customer_id)?.last_name || '')).trim(),
                    type: t.is_refunded ? 'REFUND' : 'INCOME',
                    amount: t.is_refunded ? -(t.paid_amount || 0) : (t.paid_amount || 0),
                    method: 'Synth'
                }))
                .filter(l => {
                    const d = new Date(l.date);
        ...
        return { tickets, ledger: finalLedger };
    }, [data, dateRange]);
    ...
    const totals = useMemo(() => {
        // 1. Calculate Gross Sales from Items
        // We link items to the tickets in the current filtered set
        const ticketIds = new Set(filteredData.tickets.map(t => t.id));
        const filteredItems = data?.items.filter(item => ticketIds.has(item.ticket_id)) || [];

        const grossSales = filteredItems.reduce((acc, item) => acc + (item.quantity * item.price), 0);

        // 2. Calculate Actual Revenue (Collected)
        const revenue = filteredData.tickets
            .filter(t => !t.is_refunded)
            .reduce((acc, t) => acc + (t.paid_amount || 0), 0);

        // 3. Calculate Refunds
        const refunds = filteredData.tickets
            .filter(t => t.is_refunded)
            .reduce((acc, t) => acc + (t.paid_amount || 0), 0);

        // 4. Calculate Outstanding (Only for non-voided/non-refunded tickets)
        // This is simplified as (Expected - Collected)
        const outstanding = Math.max(0, grossSales - revenue - refunds);
        ...
        return {
            ...
            outstanding,
            avgTicket: filteredData.tickets.length > 0 ? grossSales / filteredData.tickets.length : 0
        };
    ...
    const chartData = useMemo(() => {
        ...
        if (viewMode === 'operations') {
            ...
            if (chartType === 'Pie') {
                return [
                    { label: 'Collected', value: totals.revenue },
    ...
    const handleDownloadPDF = () => {
        ...
        if (viewMode !== 'customers') {
            ...
            const totalText = viewMode === 'operations'
                ? `Total Volume: ${filteredData.tickets.length} tickets`
                : `Net Revenue: ${formatCurrency(totals.netRevenue)}`;
        ...
        <div className="max-w-7xl mx-auto space-y-6 font-sans text-gray-700 pb-12">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end border-b border-gray-200 pb-4 gap-4">
                ...
                <div className="flex bg-gray-100 p-1 rounded-lg overflow-x-auto">
                    {/* Operations tab removed — focus on Financials */}
            ...
            {/* Customer Growth Chart (when customer view selected) */}
            {viewMode === 'customers' && charts?.customer_growth && charts.customer_growth.length > 0 && (
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm animate-in fade-in slide-in-from-top-4">
                    <div className="flex justify-between items-center mb-2">
                        <h3 className="font-bold text-lg text-gray-800">Customer Growth</h3>
                        <span className="text-sm text-gray-400">Last {charts.customer_growth.length} periods</span>
                    </div>
                    <SimpleChart data={charts.customer_growth.map(c => ({ label: c.label, value: c.value }))} type={'Line'} />
                </div>
            )}
